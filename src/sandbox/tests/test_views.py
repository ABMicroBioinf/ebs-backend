...
#from .factories import CompanyFactory, UserFactor
...
class CompanyViewSetTestCase(TestCase):
    def setUp(self):
        ...
        self.user.save()
        login = self.client.login(email=self.user.email, password='testpassword')
        self.assertTrue(login) 
        self.list_url = reverse('sandbox:company-list')
    
    def get_detail_url(self, company_id):
        return reverse('sandbox:company-detail', kwargs={'pk': company_id})
 ...
 """
        ...
        self.assertEqual(
            ...
            set(company['id'] for company in response.data),
